...
function sitemapEntry(segment, lastmod) {
  ...
  const slug = (segment.match(/^(?:en\/)?milestone\/(.+)$/) || [])[1];
  const isMilestone = Boolean(slug);
  ...
  // Each milestone lists both language versions (same set on both URLs).
  const alternates = isMilestone
    ? [
        ['zh-CN', `${ORIGIN}/milestone/${slug}/`],
        ['en', `${ORIGIN}/en/milestone/${slug}/`],
        ['x-default', `${ORIGIN}/milestone/${slug}/`],
      ]
