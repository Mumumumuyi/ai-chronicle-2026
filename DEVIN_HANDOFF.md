...
**目标**：为已上线站点起草各平台推广帖，站长用本人账号手动发布。产出 `marketing/launch-posts.md`（新建），未改其他任何文件。

**完成内容**：Reddit r/InternetIsBeautiful（link post 标题+自评论）、HN Show HN（朴素标题+技术向首评：Vite+React / Playwright 构建期预渲染 34 路由 / SVG 溶解滤镜 / 零后端）、Product Hunt（≤60 字符 tagline + 描述 + maker comment）、知乎 3 篇存量问题回答、V2EX 分享创造（诚实标题 + 正文 + 短板自陈）、掘金技术文大纲+开头 300 字（Playwright 预渲染主题）。每稿附建议发布时间段 + 社区规则要点（均用 web_search 查证并附出处链接）+ 首条自评论要点。

**关键决定**：
- 只写真功能：7 时代、30 里程碑页、`/reader/` 1.8 万字长卷、`/lab/` 缩放律实验室、5 种 UI 语言（长卷仅中文，英文稿已如实标注此局限）、首页时代浏览器与动效。无用户数/流量/好评等虚构数据，无「最全/权威」夸大词，全程未提赞助与收款。
- 知乎选题（均经搜索确认真实存在）：`question/550824549`（如何快速入门机器学习）、`question/2030967375349163196`（如何评价 DeepSeek V4 预览版）、「有哪些宝藏网站值得推荐？」（tardis 回答页链接已给，站长需在知乎内搜标题确认问题页 URL 再答）。三篇各自独立、间隔发布，防「同文多题」处罚。
- 需站长注意的两条硬规则：① r/InternetIsBeautiful 的 90/10 规则——若账号是新号/纯推广号需先养号或先 modmail；② V2EX 分享创造要求账号注册满 30 天，不满则该稿压后。
- 掘金文按社区规则不含站推广链接正文位、需加 AIGC 辅助声明。

**下一步（站长）**：按稿内「发布顺序建议」执行（先掘金/V2EX，再知乎，最后一周集中发英文平台）；发布前自查 Reddit 账号历史与 V2EX 账号天数；发布结果与反馈可记回本文件。
